...
export async function activate(disposables: vscode.Disposable[], globalStoragePath: string): Promise<string | null> {
  const ui = new UI(disposables, globalStoragePath);
  disposables.push(vscode.commands.registerCommand('clangd.install', async () => common.installLatest(ui)));
  disposables.push(vscode.commands.registerCommand('clangd.update', async () => common.checkUpdates(true, ui)));
...
class UI {
  ...
  get clangdPath(): string {
    ...
    if (!path.isAbsolute(p) && p.includes(path.sep) && vscode.workspace.rootPath !== undefined) {
