...
export const activateClangd = async (context: vscode.ExtensionContext): Promise<ClangdExtensionImpl> => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('clangd.restart', async () => {
      ...
      clangdContext = await ClangdContext.create(context.globalStoragePath, outputChannel);
      if (clangdContext) {
  ...
  if (vscode.workspace.getConfiguration('clangd').get<boolean>('enable')) {
    ...
    shouldCheck = vscode.workspace.getConfiguration('clangd').get<boolean>('detectExtensionConflicts') ?? false;
  }
