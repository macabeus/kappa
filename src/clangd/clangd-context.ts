...
import { ASTNode, ASTParams } from "./vscode-clangd";
import { ASTVisitor, DoubleIntAssignmentPlugin } from "../ast-visitor";

const ASTRequestMethod = "textDocument/ast";
const ASTRequestType = new vscodelc.RequestType<
  ASTParams,
  ASTNode | null,
  void
>(ASTRequestMethod);
...
export class ClangdContext implements vscode.Disposable {
  ...
  private constructor(
    ...
    vscode.commands.registerCommand("kappa.astNodeName", async () => {
      ...
      // Use the AST visitor to analyze the tree
      if (item) {
        const visitor = new ASTVisitor();

        const doubleIntAssignment = new DoubleIntAssignmentPlugin();
        visitor.registerPlugin(doubleIntAssignment);
