...
export class ASTVisitor {
  ...
  async walk(
    rootNode: ASTNode,
    context?: any,
    visitOrder: "pre-order" | "post-order" = "pre-order"
  ): Promise<void> {
  ...
  async updateDocumentFromNode(node: ASTNode): Promise<boolean> {
    try {
      ...
      if (!node.range || !node.detail) {
        console.warn(
          "Node missing range or detail information for document update"
        );
        return false;
      }
      ...
      if (success) {
        console.log(
          `Successfully updated document with new code: ${node.detail}`
        );
      } else {
...
/**
 * Abstract base class for creating AST visitor plugins
 */
export abstract class BaseASTVisitorPlugin implements ASTVisitorPlugin {
...
export class DoubleIntAssignmentPlugin extends BaseASTVisitorPlugin {
  ...
  async visit(
    ...
    if (node.detail === "=" && node.children?.[1].kind === "IntegerLiteral") {
      ...
      // Get the type of the left child using the visitor's getNodeType method
