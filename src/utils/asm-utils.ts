...
export function extractAssemblyFunction(assemblyContent: string, functionName: string): string | null {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line === `${functionName}:` || line.startsWith(`${functionName}: @`)) {
      if (functionStart === -1) {
        // If we haven't found thumb_func_start, start from the previous line to include it
        functionStart = Math.max(0, i - 1);
      }
      inFunction = true;
      continue;
    }
